...
this.viewport.offsetLeft+"px","top:-"+this.viewport.offsetTop+"px"].join(";"),src:this.viewport.getAttribute("data-load"),seamless:!0,width:+a.innerWidth+"px",height:+a.innerHeight+"px"};this.viewport.insertBefore(this.setAttributes(b,g),this.viewport.firstChild);return this};b.prototype.initNav=function(){for(var a=["<input type=text value=0 readonly><ol>","</ol><canvas></canvas>"],b=this.setAttributes(h.createElement("section"),{"class":"atomic"}),c=h.createElement("nav"),e=this.articles.length,
d;e--;)d=this.articles[e].getElementsByTagName("h1")[0],a.splice(1,0,"<li><label>"+(d.innerText||d.textContent)+"</label></li>");b.innerHTML=a.join("");c.appendChild(b);this.viewport.insertBefore(c,this.viewport.firstChild);this.atomic=new Atomic(b);return this};b.instance=new b(document.getElementsByClassName("dawn")[0]);a.Dawn=b})(window,document);